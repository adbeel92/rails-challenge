...
describe 'DELETE /users/:user_id/links/:id', type: :request do
  context 'When a user account exists' do
    ...
    context 'unauthorized' do
      it 'responds unauthorized when you are not logged in' do
        delete "/api/v1/users/#{user.id}/links/#{link.id}"
    ...
    context 'authorized' do
      ...
      context 'a successful response' do
        before(:each) do
          delete "/api/v1/users/#{user.id}/links/#{link.id}", headers: authorization_header
        end
      ...
      context 'an invalid response' do
        ...
        it 'has an no-existing link' do
          delete "/api/v1/users/#{user.id}/links/0", headers: authorization_header

          expect(response).to have_http_status(:not_found)
        end

        it 'has an unauthorized link' do
          delete "/api/v1/users/#{user.id}/links/#{another_link.id}", headers: authorization_header
